  ...
  <div class="app-container">
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
      <el-upload
        ...
        :action="upload.url + '?updateSupport=' + upload.updateSupport + '&dictType=' + upload.dictType"
...
export default {
  ...
  methods: {
    ...
    /** 查询字典类型列表 */
    getTypeList() {
      listType({ pageSize: 999 }).then(res => {
        const { list } = res.data
    ...
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = '字典项导入'
      this.upload.open = true
      this.upload.dictType = this.queryParams.dictType
      console.log(this.upload)
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download('system/dict/data/importTemplate', {}, `dict_data_template_${new Date().getTime()}.xlsx`)
    },
    // 文件上传中处理
